...
##baseline category
  ...
  colnames(housedata)
##Correlation matrix
  cormatrix <- cor(housedata[,c(1,4,5,6,7)])
  cormatrix

##Scatterplot matrix
  pairs(~ SalePrice+Age+BasementArea+LivingArea+TotalRoom, 
        data = housedata, 
...
##Least Squares Regression Analysis
  house.model<-lm(SalePrice ~ Age+BasementArea+LivingArea+TotalRoom+Garage, 
             data = housedata)
  summary(house.model)
...
##Create plot with leverage values on x-axis and 
  ...
  ##Leverage cut-offs:
  lev.cutoff.low<-2*(5+1)/200
  lev.cutoff.high<-3*(5+1)/200
